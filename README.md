...
## Project Structure
- `src/SaaS.Domain`: Core entities (`Tenant`), interfaces, and domain exceptions.
- `src/SaaS.Application`: Business logic, CQRS behaviors, DTOs, validators, and onboarding workflow services.
- `src/SaaS.Infrastructure`: EF Core implementations (`MasterDbContext` & `TenantDbContext`), `AppUser` Identity model, tenant provisioning, and repositories.
- `src/SaaS.API`: Controllers, JWT authentication setup in `Program.cs`, Swagger, and middlewares.
- `tests/SaaS.API.IntegrationTests`: HTTP integration tests for the API host and auth flows.
- Root level: `SaaS.sln`, `.gitignore`, `Dockerfile`, `docker-compose.yml`, and repository-wide docs/configuration.

## Getting Started
1. Open `src/SaaS.API/appsettings.json` and configure your `"MasterConnection"` pointing to your PostgreSQL server.
...
3. **Apply database schema explicitly**:
   ...
   ```
4. Run the API project (`dotnet run --project src/SaaS.API` or F5 in Visual Studio).
